...
--시퀀스 사용 방법 2가지
SELECT DEPTS_SEQ.CURRVAL FROM DUAL; --NEXTVAL가 한번 실행이 되고난 이후에 확인이 가능합니다.
SELECT DEPTS_SEQ.NEXTVAL FROM DUAL; --한번 NEXTVAL가 일어나면 후진은 불가능.

INSERT INTO DEPTS VALUES(DEPTS_SEQ.NEXTVAL, 'EXAMPLE');--MAXVALUE에 도달하면, 더이상 사용이 불가능
SELECT * FROM DEPTS;
--시퀀스의 수정
...
--이 테이블에 PK를 2025-00001형식으로 INSERT하려고 합니다.
--다음 값은 2025-00002형식이 됩니다.
--시퀀스를 만들고, INSERT넣을때, 위 형식처럼 값이 들어갈 수 있도록 INSERT를 넣어주세요.
CREATE SEQUENCE EMPS_SEQ
    START WITH 1 
    INCREMENT BY 1
    NOCACHE;
    
INSERT INTO EMPS (EMPS_NO, EMPS_NAME)
VALUES ('2025-' || LPAD(emps_seq.NEXTVAL, 5, '0'),'HONG' );
SELECT * FROM EMPS;

----------------------------------------------------------
--인덱스(검색을 빠르게하는 HINT역활)
...
--고유인덱스
CREATE UNIQUE INDEX 인덱스명 ON 테이블명(부착할컬럼); --PK,UK만들때 자동 생성해줍니다(PK,UK조회할 때 인덱스 효과를 받음)
